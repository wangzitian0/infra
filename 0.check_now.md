...
Change Log: [2025-12-09.storage_retain.md](docs/change_log/2025-12-09.storage_retain.md)
...
|---------|------------|---------|---------|
| `i-*` | Grey cloud (DNS-only) | Internal infra | `i-atlantis`, `i-secrets` |
...
| File | Change |
|------|--------|
| `terraform/1.nodep/network.md` | Complete rewrite with i-/x- patterns |
| `terraform/1.nodep/3.dns_and_cert.tf` | Keep x-staging, remove x-prod |
| `terraform/1.nodep/4.storage.tf` | Patch local-path path + add `local-path-retain` StorageClass |
| `terraform/1.nodep/README.md` | Update DNS + storage docs |
| `terraform/2.platform/1.postgres.tf` | Use `local-path-retain` for Postgres PVC |
| `terraform/2.platform/README.md` | Document storage workaround |
...
- 改为 Atlantis plan 成功后触发 Claude review
- 添加 `pull-requests: write` 权限让 Claude 能发评论

### 新流程
```
PR → CI 验证 → atlantis plan → (成功) → Claude review
```
...
| `.github/workflows/claude.yml` | 添加 Atlantis 成功评论触发 + write 权限 |
| `.github/workflows/terraform-plan.yml` | 只触发 atlantis plan，不触发 PTAL |
| `.github/workflows/README.md` | 更新流程图和文档 |
