...
| File | Change |
|------|--------|
| `1.bootstrap/locals.tf` | Switch infra domain map to `i-*` prefixes (env domains unchanged) |
| `1.bootstrap/2.atlantis.tf` | Atlantis URL/ingress hosts use `i-atlantis` |
| `1.bootstrap/3.dns_and_cert.tf` | Add explicit `i-*` grey-cloud A records; optional internal wildcard only when zones differ |
| `1.bootstrap/network.md` | DNS architecture rewritten for i-/x- scheme |
| `1.bootstrap/README.md` | Domain scheme updated (i-/x-) |
...
| `2.platform/2.secret.tf` | Vault ingress uses `i-secrets` |
| `2.platform/3.dashboard.tf` | Dashboard ingress uses `i-kdashboard` |
| `2.platform/4.kubero.tf` | Kubero UI ingress uses `i-kcloud` |
