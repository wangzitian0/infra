...
## DONE (History)
- [x] [2025-12-20: Vault Path SSOT Module (Issue #301)](./docs/change_log/2025-12-20.vault_ssot_module.md)
- [x] [2025-12-19: Pipeline V2 Upgrade (Dashboard SSOT & AI Review Auto-writeback)](./docs/change_log/2025-12-19.pipeline_v2_upgrade.md)
...
### 🔴 关键 (Critical)
- [ ] **资源蔓延** – 集群中并存 data, data-default, data-staging, data-prod 多个重复层级；postgresql 在 5 个命名空间运行。
- [ ] **Ingress 冲突** – `kcloud.zitian.party` 同时被 kubero-prod 和 kubero-staging 使用，路由不确定。
- [ ] **证书失败** – `wildcard-tls-internal` 签发报 Error 7003 (Cloudflare Zone ID 发现失败)。
- [ ] **中间件缺失** – kubero-staging 引用了不存在的 `portal-auth` 中间件，导致 404。

### 🟡 严重 (High)
- [ ] **存储残留** – 大量 Released 状态的 PV (旧 security/platform 卷) 未回收。
- [ ] **Atlantis 策略失效** – 定义了 `atlantis-ipallowlist` 但未挂载到 Ingress，Webhook IP 限制未开启。
- [ ] **权限合规** – `dashboard-admin` 依然直接绑定在 `cluster-admin`，未最小化。

### 🔵 中等/低 (Medium/Low)
- [ ] **Casdoor 健壮性** – `2.platform/90.casdoor-apps.tf:74-75` 不可用时静默退出，应改为报错。
- [ ] **Scope 校验** – 确认 Casdoor 是否支持硬编码的 `openid profile email`。
- [ ] **文档不一致** – `docs/ssot/db.redis.md` 描述的 Vault Agent 流程在代码中未体现。
- [ ] **Traefik 兼容性** – `platform.network.md` 中 NGINX 注解对 Traefik 无效。