...
9.  **Infisical TLS**: Set ingress `hostName` to `i-secrets.${base_domain}` so NGINX serves the issued LE cert.
10. **Atlantis Strictness**: Atlantis plan/apply now fail fast if R2/Cloudflare/VPS env vars are missing (no local-backend fallback) and TF_VARs injected for Terraform.
