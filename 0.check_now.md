...
## 1. What (Target)
Post-merge smoke test for PR #182 — verify per-commit infra-flash comment + Atlantis Plan/Apply append works end-to-end on `main`.
...
## 5. When (Timeline)
Dec 15 window → PR #182 merged. Next: run one small PR to confirm CI comment creation + Atlantis append in real workflows.
...
- [ ] Push 2 commits to a smoke PR ⇒ confirm two distinct infra-flash comments exist (each scoped to its SHA).
- [ ] Run `atlantis plan` / `apply` on the smoke PR ⇒ Plan/Apply blocks append to the correct comment with trigger + output links.
