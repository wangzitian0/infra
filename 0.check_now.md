...
| File | Change |
|------|--------|
| `1.bootstrap/network.md` | Complete rewrite with i-/x- patterns |
| `1.bootstrap/3.dns_and_cert.tf` | Keep x-staging, remove x-prod |
| `1.bootstrap/4.storage.tf` | Patch local-path path + add `local-path-retain` StorageClass |
| `1.bootstrap/README.md` | Update DNS + storage docs |
| `2.platform/1.postgres.tf` | Use `local-path-retain` for Postgres PVC |
| `2.platform/README.md` | Document storage workaround |
...
- `1.bootstrap/variables.tf`: 添加 `infisical_postgres_password`
