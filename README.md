...
- Atlantis：PR 规划阶段/`atlantis apply` 必须提供 R2、Cloudflare、VPS 环境变量（`R2_BUCKET`、`R2_ACCOUNT_ID`、`AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY`、`VPS_HOST`、`VPS_SSH_KEY`、`CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ZONE_ID`），否则会直接失败（不再降级本地 backend）。
