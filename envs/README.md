...
envs/
  staging/
    3.data/          # L3 staging data layer
  prod/
    3.data/          # L3 production data layer
```
...
| **L2** | **Platform** | **Shared** (Singleton) | `2.platform/` | Atlantis (`platform`) | Vault, Dashboard, Casdoor |
| **L3** | **Data** | **Isolated** (Per-env) | `envs/{env}/3.data/` | Atlantis (`data-staging`/`data-prod`) | Business databases in `data-{env}` |
...
- **L3 Directory Isolation**: Separate state files per environment
  - Staging: `envs/staging/3.data/` → `k3s/data-staging.tfstate`
  - Prod: `envs/prod/3.data/` → `k3s/data-prod.tfstate`
...
- **Environment Detection**: Automatic via directory structure
  - `envs/staging/3.data/` → `environment = "staging"`
  - `envs/prod/3.data/` → `environment = "prod"`

### Usage

```bash
# Staging
cd envs/staging/3.data
