    ...
    yamlencode(merge(
      {
        ...
        # All secret env/TF_VAR_* are provided via Kubernetes Secret (loadEnvFromSecrets)
        loadEnvFromSecrets = [
          kubernetes_secret.atlantis_env.metadata[0].name
        ]
...
resource "kubernetes_secret" "atlantis_env" {
  metadata {
    name      = "atlantis-env"
    namespace = kubernetes_namespace.nodep.metadata[0].name
  }
