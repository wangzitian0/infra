...
- **Namespace ownership**: `platform` namespace is created in L1 (`1.bootstrap/5.platform_pg.tf`), not L2. The Atlantis platform workflow removes stale L2 namespace state during plan (`terraform state rm kubernetes_namespace.platform`) to prevent accidental destruction.
