...
# Provides a forward-auth middleware for portals without native OIDC (e.g., Dashboard).
...
# 3) Set enable_portal_sso_gate=true and apply to gate non-OIDC portals.
...
resource "kubernetes_manifest" "portal_auth_middleware_platform" {
  ...
  manifest = {
    ...
    spec = {
      forwardAuth = {
        address             = "http://portal-auth-oauth2-proxy.${data.kubernetes_namespace.platform.metadata[0].name}.svc.cluster.local/oauth2/auth"
