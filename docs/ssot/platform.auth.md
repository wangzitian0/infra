...
### 1. 需要开发者简化版文档
**问题**: platform.auth.md 内容非常详细和技术化，面向基础设施运维者。应用开发者只想知道"我的应用如何接入 SSO"，不需要了解所有架构细节和历史问题。

**建议**:
- [ ] 新增 `platform.auth-for-developers.md` 文件，专门面向应用开发者
- [ ] 内容应包括：
  - **我的应用需要 SSO 登录吗？** 决策树（公开应用 vs 内部应用）
  - **快速接入 Casdoor OIDC** (3 步完成)：
    1. 在 Casdoor 创建应用 (或联系管理员)
    2. 配置应用的 Redirect URI
    3. 在代码中集成 OIDC 客户端库（提供各语言示例）
  - **验证 SSO 是否工作**: 测试登录流程的清单
  - **常见问题**: 重定向循环、Token 过期等
- [ ] 链接回本文档作为"深入了解架构"的入口

**受影响角色**: 应用开发者（需要 SSO）

### 2. 分治策略对开发者的影响需要明确说明
**问题**: 文档详细解释了"分治策略"（原生 OIDC vs Portal Gate），但没有明确告诉开发者"我应该选哪个"。

**建议**:
- [ ] 在 "门户级认证分治" 章节前增加 "开发者决策指南" 小节
- [ ] 提供清晰的决策树：
  - 你的应用框架支持 OIDC 吗？
    - 是 → 使用原生 OIDC（更简单、更安全）
    - 否 → 使用 Portal Gate（需要管理员配置）
  - 常见框架的 OIDC 支持情况列表
- [ ] 明确说明两种方式的开发者体验差异
...
### 3. 根密钥管理章节对开发者无用
**问题**: "根密钥管理"、"紧急绕过与恢复"等章节是运维内容，对应用开发者来说是噪音。

**建议**:
- [ ] 在文档开头增加"受众说明"
- [ ] 明确标注哪些章节是给运维者的，哪些是给开发者的
- [ ] 或者拆分为两个文档：
  - `platform.auth.md` (运维视角)
  - `platform.auth-for-developers.md` (开发者视角)
