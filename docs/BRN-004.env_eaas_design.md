...
|---------|------|
| `kube-system` | k3s 系统 |
| `secrets-infra` | Infisical + PostgreSQL (Infisical DB) |
| `data` | Redis + Neo4j |
| `kubero` | Kubero Controller + UI |
| `apps` | 用户应用（未来） |
| `observability` | SigNoz（后期） |
| `ingestion` | PostHog（后期） |
| `ingress` | Traefik（可选） |

### Staging 域名方案

使用 `staging.truealpha.club` 及其子域：

| 服务 | 域名 |
|------|------|
| Kubero UI | `api.staging.truealpha.club` |
| Infisical | `infisical.staging.truealpha.club` |
| 应用前端（未来） | `staging.truealpha.club` |
| 应用后端（未来） | `api.staging.truealpha.club` |
