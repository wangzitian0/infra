...
|------|------|------|
| **Phase 0** | k3s | - |
| **Phase 1.1** | PostgreSQL | k3s |
| **Phase 1.2** | Infisical | PostgreSQL |
| **Phase 2.1** | Redis + Neo4j | k3s |
| **Phase 3.1** | Kubero | k3s + Infisical |
| **Phase 3.2** | Kubero UI | Kubero |
...
按 `{env}.x.truealpha.club` 规范（与 Cloudflare 域名设计一致）：

| 服务 | 域名 |
|------|------|
| Kubero UI | `api.staging.x.truealpha.club` |
| Infisical | `infisical.staging.x.truealpha.club` |
| 应用前端（未来） | `staging.x.truealpha.club` |
| 应用后端（未来） | `api.staging.x.truealpha.club` |

**生产环境**则为 `truealpha.club` 和 `api.truealpha.club`（无 `.x`）。
