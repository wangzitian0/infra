...
| 阶段 | 目标 | 组件（同阶段内可并行） | 前置 |
|------|------|------------------------|------|
| **Phase 0.x** | 引导 + 密钥基线 | k3s、Infisical（后续密码统一进 Infisical） | - |
| **Phase 1.x** | 管理入口 + PaaS 底座 | Kubernetes Dashboard、Kubero、Kubero UI、平台 PostgreSQL | Phase 0.x |
| **Phase 2.x** | 数据服务 | PostgreSQL（应用库）、Neo4j、Redis、ClickHouse | Phase 0.x（如 Kubero 管理应用，建议 1.x 完成） |
