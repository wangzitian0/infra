...
|------|------------|--------------|-----------|
| **prod** | `${BASE_DOMAIN}` | `cloud.${BASE_DOMAIN}` | `signoz.${BASE_DOMAIN}` |
| **staging** | `${DOMAIN_PREFIX}.${BASE_DOMAIN}` | `cloud-${DOMAIN_PREFIX}.${BASE_DOMAIN}` | `signoz-${DOMAIN_PREFIX}.${BASE_DOMAIN}` |
| **test** | `x-test.${BASE_DOMAIN}` | `cloud-x-test.${BASE_DOMAIN}` | 不部署 |
...
> 约定：平台 PostgreSQL（Infisical/Kubero 用）在 Phase 0.x；业务 PostgreSQL 在 Phase 1.x。

### Phase 0.x：Bootstrap + 密钥基线
- k3s 集群（kubeconfig 可用）
- 平台 PostgreSQL（Infisical/Kubero 用）
- Infisical（上线后所有密码写入 Infisical）
...
├── outputs.tf
├── backend.tf
├── k3s.tf                           # k3s 引导（SSH provisioner）
│
├── terraform.tfvars.example         # 本地 tfvars 模板
│
├── phases/
│   ├── 0_1_infisical.tf             # Infisical（平台密码管理，依赖平台 PG）
│   ├── 1_1_postgresql.tf            # 业务 PostgreSQL（当前文件命名沿用旧序号）
│   ├── 1_3_dashboard.tf             # Kubernetes Dashboard
│   └── variables.tf
│
├── scripts/
...
# k3s 在 k3s.tf；平台 PG 暂存于 phases/1_1_postgresql.tf（命名沿用旧序号）；Infisical 在 phases/0_1_infisical.tf
terraform apply -var-file="staging.tfvars" -target="null_resource.k3s_server"
terraform apply -var-file="staging.tfvars" -target="module.phases.helm_release.postgresql"   # 平台 PG（Infisical/Kubero）
terraform apply -var-file="staging.tfvars" -target="module.phases.helm_release.infisical"

# 3. Phase 1.x（管理入口 + PaaS + 业务库）
terraform apply -var-file="staging.tfvars" -target="module.phases.helm_release.kubernetes_dashboard"
# 业务 PostgreSQL / Kubero / Kubero UI：待对应模块落地后追加 target

# 4. Phase 2.x（业务数据层，待补充）
# Neo4j / Redis / ClickHouse：未来新增对应模块后按需 target

# 5. Phase 3.x（观测/分析，待补充）
# SigNoz / PostHog：未来新增对应模块后按需 target
```
...
**Staging 示例**：
```
cloud-${DOMAIN_PREFIX}.${BASE_DOMAIN} → Infisical
api-${DOMAIN_PREFIX}.${BASE_DOMAIN}   → Kubero UI + API
signoz-${DOMAIN_PREFIX}.${BASE_DOMAIN} → SigNoz
posthog-${DOMAIN_PREFIX}.${BASE_DOMAIN} → PostHog
...
- [ ] Phase 1.x：平台 PostgreSQL、Kubernetes Dashboard 就绪（Kubero/Kubero UI 待补）
- [ ] Phase 2.x：业务 PostgreSQL/Neo4j/Redis/ClickHouse 就绪（按需）
- [ ] Phase 3.x：SigNoz + PostHog UI 可访问（按需）
