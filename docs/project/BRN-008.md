...
| **目录结构语义不清** | `docs/dir.md` 既是导航图又是 SSOT 定义 | 职责混合 |
...
| 层级 | 当前实现 | 目标状态 |
|------|----------|----------|
| **L1 Bootstrap** | `vault_postgres_password` via GitHub Secret → TF_VAR | ✅ 保持 |
| **L2 Platform** | Vault storage backend 使用该密码 | ✅ 密码由 TF `random_password` 生成更佳 |
| **L3 Data** | 业务 PostgreSQL/Redis/Neo4j 密码 | 🎯 **改进**：TF 生成 → Vault 存储 → Pod 注入 |

**改进方案**：
```hcl
resource "random_password" "db_password" { ... }
# → vault_generic_secret → k8s Secret via Vault Agent
```
应用层无需知道密码，Vault Agent Injector 自动注入 `/vault/secrets/db`。
...
**记录位置**：`docs/ssot/vars.md` § Bootstrap Secrets (L0/L1)
...
| **代码审查** | Atlantis 成功后自动 | Claude GitHub Action | `.github/workflows/claude.yml` |
...
└── ssot/                   # [NEW] 话题式 SSOT
    ├── README.md           # SSOT 索引
    ├── dir.md              # 目录结构图 (从 docs/ 移入)
    ├── vars.md             # 环境变量（非密钥）
    ├── secrets.md          # 密钥分层与管理
    ├── pipeline.md         # 自动化/手动流程
    ├── db.md               # 数据库能力分布
    ├── auth.md             # 认证与授权
    └── network.md          # 网络与域名
```
...
├─────────────────────────────────────────────────────────────┤
│  • K3s Cluster           密钥来源：GitHub Secrets           │
│  • Platform PostgreSQL   密钥来源：GitHub Secrets           │
│  • Atlantis CI           认证：Basic Auth (不走 SSO)        │
│  • DNS/Cert              密钥来源：GitHub Secrets           │
│  • Storage Class                                            │
└─────────────────────────────────────────────────────────────┘
         ▲ 不依赖任何其他层
         │
...
├──────────────────┤  ├──────────────────┤  ├──────────────────┤
│ • Vault          │  │ • Business PG    │  │ • 业务应用       │
│ • Casdoor (SSO)  │  │ • Redis          │  │                  │
│ • Dashboard      │  │ • Neo4j          │  │                  │
│ • Kubero         │  │ • ClickHouse     │  │                  │
├──────────────────┤  ├──────────────────┤  ├──────────────────┤
│ 依赖: L1 PG      │  │ 依赖: L2 Vault   │  │ 依赖: L2 + L3    │
│ 密钥: Vault 存储 │  │ 密钥: Vault 注入 │  │ 密钥: Vault 注入 │
│ 认证: Casdoor    │  │ 认证: N/A        │  │ 认证: Casdoor    │
└──────────────────┘  └──────────────────┘  └──────────────────┘
```
...
| **L2** | ✅ 遵守 | 密钥存 Vault，认证走 Casdoor（自身除外） |
| **L3** | ✅ 遵守 | 密钥由 Vault 注入 |
...
| OAuth Client | ID/Secret | GitHub OAuth App | `secret/data/oauth` | Casdoor |
...
```
┌─────────────────────────────────────────────────────────────┐
│  L1 Bootstrap — Platform PostgreSQL [需部署]                │
├─────────────────────────────────────────────────────────────┤
...
**待办**：
1. 在 `1.bootstrap/5.platform_pg.tf` 或 `2.platform/0.postgres.tf` 部署 PostgreSQL
2. 修改 `2.platform/2.secret.tf`，将 Vault storage 从 Raft 改为 PostgreSQL
3. 执行 Vault 数据迁移
...
| Casdoor DB | L1 | `platform` | (与 Vault 共用) |
...
| 文件 | 核心问题 | 关键内容 |
|------|----------|----------|
| `dir.md` | 项目结构 | 目录树、Layer 定义、Namespace 注册 |
| `vars.md` | 非密钥变量 | TF_VAR 列表、默认值、Feature Flags |
| `secrets.md` | 密钥管理 | 四层模型、1Password 清单、Vault Path |
| `pipeline.md` | 流程汇总 | L1 手动 vs L2+ GitOps、灾备 |
| `db.md` | 数据库分布 | Platform PG (L1) vs Business DBs (L3) |
| `auth.md` | 统一认证 | Casdoor SSO、服务接入矩阵 |
| `network.md` | 域名规则 | Internal vs Env 模式 |
...
| `docs/ssot/dir.md` | 从 `docs/dir.md` 移动 | 目录结构图 |
| `docs/ssot/vars.md` | 从 `variables.tf` + DD-001 提取 | 环境变量清单 |
| `docs/ssot/secrets.md` | 从 DD-001、README 提取 | 密钥分层 + 1Password |
| `docs/ssot/pipeline.md` | 从 `.github/workflows/README.md` 提取 | 流程汇总 |
| `docs/ssot/db.md` | 从 `3.data/README.md`、BRN-004 提取 | 数据库能力 |
| `docs/ssot/auth.md` | 从 `2.platform/README.md` 提取 | 认证能力 |
| `docs/ssot/network.md` | 从 `1.bootstrap/README.md`、`network.md` 提取 | 网络规则 |
...
| 操作 | 路径 | 目标 |
|------|------|------|
| 移动 | `docs/dir.md` | `docs/ssot/dir.md` |
| 移动 | `docs/BRN-004.env_eaas_design.md` | 归档或合并 |

### 5.4 需要更新的引用

文件内部链接需要更新：
- `AGENTS.md`: `docs/dir.md` → `docs/ssot/dir.md`
- `docs/README.md`: `./dir.md` → `./ssot/dir.md`
- 所有指向 `docs/dir.md` 的文件
