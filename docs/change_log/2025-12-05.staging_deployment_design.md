...
flowchart TB
    ...
    Terraform --> Providers["providers.tf (新)"]
    Terraform --> Variables["variables.tf"]
    Terraform --> Locals["locals.tf (新)"]
    Terraform --> Tfvars["staging.tfvars (新)"]

    Terraform --> Phases["phases/"]
    Phases --> P0["0_k3s.tf"]
    Phases --> P11["1_1_postgresql.tf"]
    Phases --> P12["1_2_vault.tf"]
    Phases --> P21["2_1_databases.tf"]
    Phases --> P31["3_1_kubero.tf"]
    Phases --> P32["3_2_kubero_ui.tf"]

    Terraform --> Modules["modules/"]
    Modules --> K8s["kubernetes/"]
    K8s --> Namespaces["namespaces.tf"]
    Modules --> Helm["helm_releases/"]
```
...
3. Phase 1.2: Vault （依赖 1.1）
4. Phase 2.1: Redis + ArangoDB
5. Phase 3.1: Kubero （依赖 Vault）
