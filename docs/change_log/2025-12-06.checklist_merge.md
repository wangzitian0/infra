...
## Phase 提示（phase 内无依赖）
- Phase 0.x：k3s + 平台 PostgreSQL（Vault/Kubero 用）+ Vault（后续密码集中到 Vault）
- Phase 1.x：Kubernetes Dashboard、Kubero、Kubero UI、业务 PostgreSQL
- Phase 2.x：数据服务（ArangoDB、Redis、ClickHouse 等）
...
## 文件快速检查
- Terraform layers 存在：`1.bootstrap/1.k3s.tf`、`1.bootstrap/5.platform_pg.tf`、`2.platform/2.secret.tf`、`2.platform/3.dashboard.tf`
- 变量定义：`1.bootstrap/variables.tf`、`2.platform/variables.tf` 包含 k3s/Vault/数据库相关参数
...
## 后续
- Phase 0.x 完成后，推进 Phase 1.x（Dashboard/Kubero/平台 PG）；后续密码通过 Vault 管理。
