...
## Summary
Aligned Terraform DNS records and ingress hosts with the internal-domain (no prefix) convention; infra endpoints now live on the internal domain (e.g., `secrets.zitian.party`), proxied for 443 services, with `k3s` left DNS-only.

## Changes

### L1 (1.bootstrap)
- `locals.tf`: Domain map uses prefix-less internal hosts; env domain mapping unchanged.
- `2.atlantis.tf`: Atlantis URL/ingress hosts switched to `atlantis.<internal_domain>` (e.g., `atlantis.zitian.party`).
...
### L2 (2.platform)
- `2.secret.tf`: Vault ingress/TLS/output hosts updated to `secrets.<internal_domain>` (e.g., `secrets.zitian.party`).
