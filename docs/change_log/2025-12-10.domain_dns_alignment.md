...
## Summary
Aligned Terraform DNS records and ingress hosts with the internal-domain (no prefix) convention; infra endpoints now live on the internal domain (e.g., `secrets.zitian.party`), proxied for 443 services, with `k3s` left DNS-only. When `internal_zone_id` is not provided and `INTERNAL_DOMAIN` differs, Terraform now auto-resolves the Cloudflare zone for the internal domain.
