# 管理密钥

> **场景**：应用需要管理 API Keys、Tokens、密钥等敏感信息
> **预计时间**：20 分钟
> **前置条件**：已完成[部署第一个应用](./02.first-app.md)

---

## 概述

**为什么需要 Vault？**
- ✅ **集中管理**：所有密钥统一存储，避免散落在代码、配置文件
- ✅ **访问控制**：每个应用只能访问自己的密钥
- ✅ **自动注入**：无需手动配置环境变量
- ✅ **审计日志**：追踪密钥的访问记录

**Vault 访问地址**：`https://secrets.zitian.party`

---

## 常见场景

### 场景 1: 第三方 API Keys

应用需要调用外部 API（如 OpenAI、Stripe、SendGrid）。

### 场景 2: 应用间通信 Token

微服务间需要使用 JWT token 或 API key 进行认证。

### 场景 3: 加密密钥

应用需要加密/解密数据的密钥（如 AES key）。

---

## 快速开始：添加一个 API Key

### 步骤 1: 确定密钥存储路径

Vault 使用路径组织密钥，推荐按应用划分：

```
secret/data/my-app/openai
secret/data/my-app/stripe
secret/data/my-app/jwt-secret
```

### 步骤 2: 联系运维添加密钥到 Vault

**方式 1: 通过运维团队**（推荐）

提供以下信息：
- 应用名称：`my-app`
- 密钥路径：`secret/data/my-app/openai`
- 密钥内容（通过安全渠道传递，如 1Password）

**方式 2: 自己通过 Vault UI 添加**（如果有权限）

1. 访问 `https://secrets.zitian.party`
2. 使用 SSO 登录
3. 导航到 `secret/my-app/`
4. 点击 "Create secret"
5. 添加 key-value pairs：
   ```
   api_key: sk-xxxxxxxxxx
   organization_id: org-xxxxxxxxxx
   ```

### 步骤 3: 配置 Vault Role（一次性）

如果这是你的第一个密钥，需要联系运维创建 Vault Role（参考[数据库指南](./03.database.md#a1-联系运维创建-vault-role)）。

**Policy 示例**：

```hcl
resource "vault_policy" "myapp" {
  name = "myapp"
  policy = <<-EOT
    path "secret/data/my-app/*" {
      capabilities = ["read"]
    }
  EOT
}
```

注意：使用 `my-app/*` 通配符，这样所有 `secret/data/my-app/` 下的密钥都可以访问。

### 步骤 4: 在应用中注入密钥

在 Deployment 添加 Vault annotations：

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: my-app
spec:
  template:
    metadata:
      annotations:
        vault.hashicorp.com/agent-inject: "true"
        vault.hashicorp.com/role: "myapp"
        # OpenAI API Key
        vault.hashicorp.com/agent-inject-secret-openai: "secret/data/my-app/openai"
        vault.hashicorp.com/agent-inject-template-openai: |
          {{- with secret "secret/data/my-app/openai" -}}
          export OPENAI_API_KEY="{{ .Data.data.api_key }}"
          export OPENAI_ORG_ID="{{ .Data.data.organization_id }}"
          {{- end }}
    spec:
      serviceAccountName: my-app
      containers:
      - name: my-app
        command: ["/bin/sh", "-c"]
        args:
          - |
            source /vault/secrets/openai
            exec node server.js
```

### 步骤 5: 在代码中使用

**Node.js 示例**：

```javascript
const { Configuration, OpenAIApi } = require("openai");

const configuration = new Configuration({
  apiKey: process.env.OPENAI_API_KEY,
  organization: process.env.OPENAI_ORG_ID,
});

const openai = new OpenAIApi(configuration);
```

---

## 多个密钥的管理

### 方式 1: 单个 Secret 包含多个 Key

**Vault 存储**：
```json
// secret/data/my-app/api-keys
{
  "openai_api_key": "sk-xxx",
  "stripe_secret_key": "sk_test_xxx",
  "sendgrid_api_key": "SG.xxx"
}
```

**Annotations**：
```yaml
vault.hashicorp.com/agent-inject-secret-api-keys: "secret/data/my-app/api-keys"
vault.hashicorp.com/agent-inject-template-api-keys: |
  {{- with secret "secret/data/my-app/api-keys" -}}
  export OPENAI_API_KEY="{{ .Data.data.openai_api_key }}"
  export STRIPE_SECRET_KEY="{{ .Data.data.stripe_secret_key }}"
  export SENDGRID_API_KEY="{{ .Data.data.sendgrid_api_key }}"
  {{- end }}
```

### 方式 2: 按服务分离 Secret

**Vault 存储**：
```
secret/data/my-app/openai
secret/data/my-app/stripe
secret/data/my-app/sendgrid
```

**Annotations**：
```yaml
# OpenAI
vault.hashicorp.com/agent-inject-secret-openai: "secret/data/my-app/openai"
vault.hashicorp.com/agent-inject-template-openai: |
  {{- with secret "secret/data/my-app/openai" -}}
  export OPENAI_API_KEY="{{ .Data.data.api_key }}"
  {{- end }}

# Stripe
vault.hashicorp.com/agent-inject-secret-stripe: "secret/data/my-app/stripe"
vault.hashicorp.com/agent-inject-template-stripe: |
  {{- with secret "secret/data/my-app/stripe" -}}
  export STRIPE_SECRET_KEY="{{ .Data.data.secret_key }}"
  {{- end }}

# SendGrid
vault.hashicorp.com/agent-inject-secret-sendgrid: "secret/data/my-app/sendgrid"
vault.hashicorp.com/agent-inject-template-sendgrid: |
  {{- with secret "secret/data/my-app/sendgrid" -}}
  export SENDGRID_API_KEY="{{ .Data.data.api_key }}"
  {{- end }}
```

**推荐**：方式 2 更清晰，但会增加 annotations 数量。对于密钥较少的应用，方式 1 更简洁。

---

## 密钥轮换

### 场景：API Key 泄露，需要更新

**步骤 1: 在 Vault 更新密钥**

通过 Vault UI 或联系运维更新对应 secret 的值。

**步骤 2: 重启应用 Pod**

Vault Agent 在 Pod 启动时注入密钥，需要重启才能加载新值：

```bash
kubectl rollout restart deployment/my-app -n apps-staging
```

**步骤 3: 验证新密钥**

查看 Pod 日志确认使用了新的 API Key。

---

## 配置文件类密钥

### 场景：需要注入 JSON 配置文件

例如 Google Cloud Service Account Key（JSON 格式）。

**Vault 存储**：
```json
// secret/data/my-app/gcp-sa
{
  "key_json": "{\"type\":\"service_account\",\"project_id\":\"xxx\",...}"
}
```

**Annotations**（注入为文件）：

```yaml
vault.hashicorp.com/agent-inject-secret-gcp-sa: "secret/data/my-app/gcp-sa"
vault.hashicorp.com/agent-inject-template-gcp-sa: |
  {{- with secret "secret/data/my-app/gcp-sa" -}}
  {{ .Data.data.key_json }}
  {{- end }}
```

**应用中使用**：

```javascript
const { GoogleAuth } = require('google-auth-library');

const auth = new GoogleAuth({
  keyFilename: '/vault/secrets/gcp-sa',
});
```

---

## 环境特定密钥

### Staging 和 Prod 使用不同的 API Key

**Vault 路径规划**：
```
secret/data/my-app/staging/openai
secret/data/my-app/prod/openai
```

**方式 1: 在 Annotations 中硬编码环境**

Staging Deployment:
```yaml
vault.hashicorp.com/agent-inject-secret-openai: "secret/data/my-app/staging/openai"
```

Prod Deployment:
```yaml
vault.hashicorp.com/agent-inject-secret-openai: "secret/data/my-app/prod/openai"
```

**方式 2: 使用环境变量**（推荐）

在 Vault template 中读取环境变量：

```yaml
vault.hashicorp.com/agent-inject-template-openai: |
  {{- with secret (printf "secret/data/my-app/%s/openai" (env "ENVIRONMENT")) -}}
  export OPENAI_API_KEY="{{ .Data.data.api_key }}"
  {{- end }}
```

在 Kubero 设置环境变量 `ENVIRONMENT=staging` 或 `ENVIRONMENT=prod`。

---

## 故障排查

### 问题 1: 无法读取密钥 - permission denied

**错误日志**：
```
Error: permission denied at path "secret/data/my-app/openai"
```

**原因**：Vault Policy 未授权该路径。

**解决**：
1. 检查 Policy 配置是否包含该路径
2. 确认路径拼写正确（注意 `secret/data/` 前缀）
3. 联系运维更新 Policy

### 问题 2: 密钥文件为空

**原因**：Template 语法错误或 Secret 不存在。

**排查**：
1. 查看 Vault Agent init 日志：
   ```bash
   kubectl logs <pod-name> -c vault-agent-init
   ```
2. 在 Vault UI 验证 Secret 存在
3. 检查 Template 语法（Go template）

### 问题 3: 密钥更新后应用仍用旧值

**原因**：Vault Agent 只在 Pod 启动时注入一次。

**解决**：重启 Deployment（见[密钥轮换](#密钥轮换)）。

### 问题 4: Vault UI 登录后看不到密钥

**原因**：Policy 未授权或使用了错误的 Role。

**解决**：
1. 使用 OIDC 登录时，需要手动输入 role name（如 `reader`）
2. 确认该 Role 的 Policy 包含所需路径
3. 联系运维检查权限配置

---

## 最佳实践

### 1. 密钥路径命名规范

推荐结构：
```
secret/data/<app-name>/<environment>/<service>
```

示例：
```
secret/data/my-app/prod/openai
secret/data/my-app/staging/stripe
secret/data/backend/prod/jwt-secret
```

### 2. 最小权限原则

每个应用的 Policy 只授权必需的路径：

```hcl
# 好的实践
path "secret/data/my-app/*" {
  capabilities = ["read"]
}

# 不推荐
path "secret/data/*" {
  capabilities = ["read"]
}
```

### 3. 避免在代码中硬编码

❌ **错误做法**：
```javascript
const apiKey = "sk-xxxxxxxxxx"; // 硬编码
```

✅ **正确做法**：
```javascript
const apiKey = process.env.OPENAI_API_KEY; // 从环境变量读取
```

### 4. 使用配置文件模板

对于复杂配置，使用 Vault template 生成完整配置文件：

```yaml
vault.hashicorp.com/agent-inject-template-config: |
  {{- with secret "secret/data/my-app/config" -}}
  {
    "database": {
      "host": "{{ .Data.data.db_host }}",
      "password": "{{ .Data.data.db_password }}"
    },
    "api": {
      "key": "{{ .Data.data.api_key }}"
    }
  }
  {{- end }}
```

---

## 下一步

现在你的应用可以安全地管理密钥了！🎉

根据你的需求，继续探索：

| 需求 | 下一步阅读 |
|------|----------|
| **需要用户登录功能** | [接入 SSO](./05.sso.md) |
| **需要监控和日志** | [监控和分析](./06.observability.md) |

---

## 参考

- **Vault 架构详解** - [platform.secrets.md](../ssot/platform.secrets.md)
- **Vault 接入机制** - [db.vault-integration.md](../ssot/db.vault-integration.md)
- **认证与授权** - [platform.auth.md](../ssot/platform.auth.md)

---

*Last updated: 2025-12-21*
