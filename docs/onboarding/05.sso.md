...
```mermaid
graph TD
    A[应用需要用户登录?] -->|是| B{应用框架支持 OIDC?}
    A -->|否| Z[不需要 SSO]

    B -->|是| C[使用原生 OIDC]
    B -->|否| D[使用 Portal Gate]

    C --> E[阅读: 原生 OIDC 接入]
    D --> F[联系运维配置 Portal Gate]
```
...
CASDOOR_ISSUER=https://sso.zitian.party
...
1. 访问你的应用登录页
2. 点击登录按钮
3. 应该跳转到 Casdoor 登录页（`https://sso.zitian.party`）
4. 选择 GitHub 或密码登录
5. 授权后跳转回应用
6. 验证获取到用户信息
...
```mermaid
graph LR
    User[用户] -->|访问| Traefik[Ingress]
    Traefik -->|未登录| OAuth2Proxy[OAuth2-Proxy]
    OAuth2Proxy -->|重定向| Casdoor[Casdoor 登录]
    Casdoor -->|登录成功| OAuth2Proxy
    OAuth2Proxy -->|设置 Cookie + 转发请求| App[应用]
    App -->|响应| User
```
