...
- [ ] Cloudflare：`x-staging.truealpha.club` 和 `api-x-staging.truealpha.club` 等 A 记录指向 VPS
- [ ] SSL 证书：通配符 `*.truealpha.club`（一级域名）
- [ ] R2：state key 配置为 `staging/terraform.tfstate`
- [ ] GitHub Secrets：R2 凭据 + VPS SSH key
- [ ] 初始化：按 Phase 0.x → 1.x → 2.x → 3.x 推进，同一 phase 内可并行
