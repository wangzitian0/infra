...
## Where
- terraform/backend.tf：由 `backend.tf.example` 复制并填入 R2 参数。
...
## How
1) R2 backend：复制 `terraform/backend.tf.example` 为 `terraform/backend.tf`，替换 `<r2-bucket>`、`<account>`。R2 无锁，若需锁改用 S3+DynamoDB/TF Cloud。  
...
6) 本地干跑/执行：  
   - tfvars：`cp terraform/terraform.tfvars.example terraform/terraform.tfvars` 填值 → `cd terraform && terraform init && terraform plan && terraform apply`  
   - .env：`cp terraform/.env.example terraform/.env` 填值 → `set -a; source terraform/.env; set +a; cd terraform; terraform init && terraform plan && terraform apply`  
   - plan 不会执行 provisioner；apply 才会连接 VPS。`TF_VAR_ssh_private_key` 需保留换行，可用 `\n` 逃逸或 tfvars 多行。

## hint
- R2 需先建 bucket + API Token（S3 权限），填入 backend.tf 与 Secrets。  
