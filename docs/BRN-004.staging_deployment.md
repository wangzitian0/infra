...
> 约定：Infisical 的 PostgreSQL 是平台库；后续业务库在 Phase 2.x 单独部署。
...
### Phase 1.x：管理入口 + PaaS
- 平台 PostgreSQL（Infisical/Kubero 使用，独立于业务库）
...
│   ├── 1_1_postgresql.tf            # 平台 PostgreSQL（Infisical/Kubero）
...
# k3s 在 k3s.tf；Infisical 在 phases/0_1_infisical.tf（依赖平台 PG）
terraform apply -var-file="staging.tfvars" -target="null_resource.k3s_server"
terraform apply -var-file="staging.tfvars" -target="module.phases.helm_release.postgresql"
...
# Kubero/Kubero UI 尚未落地，待后续补充

# 4. Phase 2.x（业务数据层，待补充）
# 业务 PostgreSQL / Neo4j / Redis / ClickHouse：未来新增对应模块后按需 target
