...
### Phase 0: k3s Bootstrap（已完成）
- 通过 Terraform SSH provisioner 在 VPS 安装 k3s
- 生成 kubeconfig

### Phase 1: 基础设施
- **1.1** PostgreSQL (Infisical DB) → `iac` ns
- **1.2** Infisical → `iac` ns（依赖 1.1）

### Phase 2: 数据层
- **2.1** Redis + Neo4j → `data` ns（可并行）

### Phase 3: 应用平台
- **3.1** Kubero Controller → `kubero` ns（依赖 Infisical）
- **3.2** Kubero UI → `kubero` ns（依赖 3.1）

### Phase 4: 可观测性（staging/prod）
- **4.1** SigNoz → `observability` ns
- **4.2** PostHog → `ingestion` ns
...
│
├── phases/
│   ├── 0_k3s.tf                    # k3s 安装（从 main.tf 搬来）
│   ├── 1_1_postgresql.tf           # PostgreSQL
│   ├── 1_2_infisical.tf            # Infisical
│   ├── 2_1_databases.tf            # Redis + Neo4j
│   ├── 3_1_kubero.tf               # Kubero Controller
│   ├── 3_2_kubero_ui.tf            # Kubero UI
│   ├── 4_1_signoz.tf               # SigNoz（conditional）
│   ├── 4_2_posthog.tf              # PostHog（conditional）
...
# 4. Phase 1.2
terraform apply -var-file="staging.tfvars" \
  -target="module.infisical"

# 5. Phase 2.1
terraform apply -var-file="staging.tfvars" \
  -target="module.redis,module.neo4j"

# 6. Phase 3.1
terraform apply -var-file="staging.tfvars" \
  -target="module.kubero"

# 7. Phase 3.2
terraform apply -var-file="staging.tfvars" \
  -target="module.kubero_ui"

# 8. Phase 4（可选）
terraform apply -var-file="staging.tfvars" \
  -target="module.signoz,module.posthog"
```
...
### Phase 执行
- [ ] Phase 0：k3s 运行，kubeconfig 可用
- [ ] Phase 1.1：PostgreSQL pod running
- [ ] Phase 1.2：Infisical UI 可访问
- [ ] Phase 2.1：Redis + Neo4j pod running
- [ ] Phase 3.1：Kubero controller pod running
- [ ] Phase 3.2：Kubero UI 可访问
- [ ] Phase 4：SigNoz + PostHog UI 可访问（可选）
